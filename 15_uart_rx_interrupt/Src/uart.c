...
void uart2_rx_interrupt_init(void)
{
	// Configuración de pines TX:PA2 y RX:PA3
	RCC->AHB2ENR |= AHB2ENR_GPIOAEN; // clk bus
	GPIOA->MODER |= (MODER_MODE3_1);	// moder: 10
	GPIOA->MODER &= ~(MODER_MODE3_0); // es el modo alternativo
	GPIOA->AFR[0] |= (USART2_AFSEL3_0|USART2_AFSEL3_1|USART2_AFSEL3_2); // AF7
	GPIOA->AFR[0] &= ~(USART2_AFSEL3_3); // AF7
	...
	// Habilitar RX para interrupción
	USART2->CR1 |= CR1_RXNEIE;

	// Habilitar TX para interrupción
	//USART2->CR1 |= CR1_TXEIE;

	// Habilitar UART2 en NVIC
